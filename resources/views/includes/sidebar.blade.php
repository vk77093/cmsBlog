<div class="col-md-4 col-xl-3">
              ...
              <input type="text" class="form-control" name="search" value="{{request()->query('search')}}" placeholder="Search">
                ...
                @foreach ($postSingle as $postsData)
<a class="media text-default align-items-center mb-5" href="{{route('blog.show',$postsData->id)}}">
<img class="rounded w-65px mr-4" src="{{asset($postsData->image)}}">
<p class="media-body small-2 lh-4 mb-0">{{$postsData->title}}</p>
                </a>
                @endforeach
                ...
                <div class="gap-multiline-items-1">
                    @foreach ($tags as $tag)
                <a class="badge badge-secondary" href="{{route('blog.tag',$tag->id)}}">{{$tag->tag_name}}</a>
                    @endforeach
